...
def analyze_video_chunks(high_quality_chunks, low_quality_chunks, model):
    ...
    for gt_chunk, pred_chunk in tqdm(zip(high_quality_chunks, low_quality_chunks), total=len(high_quality_chunks)):
        ...
        for gt_frame, pred_frame in zip(gt_frames, pred_frames):
            # Get predictions - using classes 0 (pedestrian) and 2 (car) for VisDrone
            gt_results = model(gt_frame, classes=[2], verbose=False)
            pred_results = model(pred_frame, classes=[2], verbose=False)
...
def write_results_to_file(results, sequences, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"VisDrone Tracking Quality Analysis Results\n")
...
def main():
    ...
    output_file = f"visdrone_tracking_analysis_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
