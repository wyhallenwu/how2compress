...
def write_results_to_file(results, sequences, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"Pose Estimation Quality Analysis Results\n")
        ...
        # Write method comparison summary
        f.write("METHOD COMPARISON SUMMARY\n")
        f.write("="*80 + "\n\n")
        
        # for metric in ['oks', 'pck_0.2', 'pck_0.5', 'mpjpe_normalized', 'mpjpe_pixels']:
        #     f.write(f"{metric.upper()}:\n")
        #     f.write("-"*40 + "\n")
        #     for seq_name in sequences:
        #         if seq_name in results['ours'] and seq_name in results['accmpeg']:
        #             ours_val = results['ours'][seq_name][metric]
        #             accmpeg_val = results['accmpeg'][seq_name][metric]
        #             diff = ours_val - accmpeg_val
        #             f.write(f"{seq_name}: Ours {ours_val:.4f} vs Accmpeg {accmpeg_val:.4f} (diff: {diff:+.4f})\n")
        #     f.write("\n")
...
def main():
    ...
    # Define paths
    high_quality_base = Path("/how2compress/data/UNI25CHUNK")
    low_quality_base = Path("/how2compress/video-result")
    uniqp_base = Path("/how2compress/data/UNI30CHUNK")  # Directory containing UNI QP videos
    ...
    for seq_name in sequences:
        ...
        aq_seq_folder = high_quality_base / seq_name
        if not aq_seq_folder.exists():
            print(f"Warning: Sequence folder not found: {aq_seq_folder}")
            continue
        ...
        # Get AQ chunks
        aq_chunks = sorted(list(aq_seq_folder.glob("aq-0*.mp4")))
        if not aq_chunks:
    ...
    output_file = f"pose_analysis_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
