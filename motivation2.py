...
parser.add_argument("--qp", type=int, help="qp value")
...
r = os.path.join(results_root, f"motivation2-{dataset}-{args.qp}.txt")
# rd = os.path.join(results_root, f"motivation2-{dataset}-{qp}.txt")
...
for seq in DATASET:
    ...
    for images, labels, indices in tqdm(dataloader, desc=f"{seq} val"):
        ...
        compressed_images, sizes, enc_frames = dataset.enc_and_ret_val(
            indices, selections, DEVICE, args.qp, args.qp
        )
...
with open(r, "a") as f:
    for mAP, frame_size, t in zip(mAPs, frames_size, times):
        f.write(f"{mAP.map50_95},{mAP.map75},{mAP.map50},0,0,0,{frame_size},{t}\n")
