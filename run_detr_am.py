...
model.load_state_dict(
    torch.load(
        ...
        # "/how2compress/pretrained/train/exp-ddp-30-45-accmpeg-1713/2-best.pth"
        # "/how2compress/pretrained/train/exp-ddp-30-45-accmpeg-1704/1-best.pth"
        # "/how2compress/pretrained/train/exp-ddp-30-45-accmpeg-1702/3-best.pth"
        args.model
    )
)
model.to(DEVICE)
...
r = os.path.join(results_root, f"eval30-45-accmpeg-{dataset}-detr-1.txt")
rd = os.path.join(results_root, f"decisions30-45-accmpeg-{dataset}-detr-1.txt")
...
for seq in DATASET:
    ...
    for images, labels, indices in tqdm(dataloader, desc=f"{seq} val"):
        ...
        compressed_images, sizes, enc_frames = dataset.enc_and_ret_val(
            indices, selections, DEVICE, qmin=31, qmax=40
        )
        # cv2.imwrite(f"graph/{count:06d}-accmpeg.png", enc_frames[0])
        # break
