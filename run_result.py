...
r = os.path.join(results_root, f"eval30-45-{dataset}-h264.txt")
rd = os.path.join(results_root, f"decisions30-45-{dataset}-h264.txt")
...
for seq in DATASET:
    ...
    for images, labels, indices in tqdm(dataloader, desc=f"{seq} val"):
        ...
        cv2.imwrite(f"graph/{count:06d}-ours.png", enc_frames[0])
        # break
